The purpose of this project is to create an inventory database for magic the gathering card sellers,
	 create tools to manage the inventory, and  analyize market data.
...
This file contains;
	...
	Tools-  Price_Collector - a notebook that loops through cards sets and records individual card
		prices attached to todays date and records to the "price" database. reords thousands of rows 
		of data for future use.
	
		Add Inventory - a note book used to conect to inventory data base and add inventory.
	...
	Seller_Scan - contains a notebook to scrape the listed card price of a sellers inventory on TCG player and compare it to market price, 
				saves data and reports the cards most under market price
	

	Reports - Set Regression is a notebook used to acess collected price data and show how a card set is moving in price. shows graphs and r values
	...
	Machine_learning - folder conatians a machine learning project to make price predictions. 

	Terminal App - a comand terminal interface used to querry the data bases, and run the tools created in notebooks. to run program
			  python command.py
